...
const unsigned TEST_PREC  = 128;	// Double-precision in GiNaC, should not be changed
const unsigned TEST_ERROR = 126;	// Acceptable error in comparison
...
protected:
	...
	std::unique_ptr<LiSK::LiSK<cln::cl_N>> _lisk = make_unique<LiSK::LiSK<cln::cl_N>>(MAX_WEIGHT, TEST_PREC);
...
TEST_P(LiSKTest, ClassicalsLogarithms){
	...
	for(int i=0; i<x.size(); ++i){
		...
		auto Lres = this->_lisk->Li(w,x[i].to_cl_N());
	
		EXPECT_NEAR(cln::double_approx(cln::realpart(Gres)),
			cln::double_approx(cln::realpart(Lres)),cln::double_approx(error));
		EXPECT_NEAR(cln::double_approx(cln::imagpart(Gres)),
			cln::double_approx(cln::imagpart(Lres)),cln::double_approx(error));
	}
}


// Set weights for classical polylogarithms
INSTANTIATE_TEST_CASE_P(ArbPrec, LiSKTest, TEST_VALUES);
...
TEST_F(LiSKTest, Li22){
	...
	for(int i=0; i<x.size(); ++i){
		...
		auto Lres = this->_lisk->Li22(x[i].first.to_cl_N(),x[i].second.to_cl_N());
